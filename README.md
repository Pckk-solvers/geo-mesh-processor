...
> このブロックは、プロジェクトを進める中で得た気づきや学びを、振り返りと今後のためにまとめるものです。

### GUI設計に関する気づき

本プロジェクトでは、ランチャー画面から各種機能を呼び出す設計としています。初期は、各機能ごとに個別にアプリ画面（GUI）を実装していましたが、3つ目の機能を開発している過程で「GUI部分のテンプレートをクラスとしてまとめ、共通部分を継承する」設計の重要性に気づきました。これにより、画面レイアウトや操作性の一貫性を確保しやすくなります。

また、今回特に成長を感じた点は、「GUI（表示・入力）」と「実際の処理」を分離する意識を持って実装できたことです（まだGUIクラスにメソッドを書きすぎている課題は残っていますが…）。

モジュール分割については、初期は特に意識せず進めていたため、結果的に複数のGUIスクリプトがモジュール内に散在し、後の作業や管理がやや煩雑になりました。今後は機能ごとにディレクトリやモジュールを意識して整理していく必要性を感じています。

また、「使い方ガイド」をスクリプト内に直接埋め込んでいましたが、ガイド部分をテキストファイル等の外部ファイルで管理し、動的に読み込む方式にすることで、ガイドの修正や多言語対応がしやすくなると実感しました。これにより、ガイドだけを簡単に編集でき、コードの修正や再配布なしで内容をアップデートできるのは大きなメリットです。

### geopandasとpyprojの面積計算

今回のプロジェクトでは、主に緯度経度（地理座標系）データを扱いました。面積計算にはgeopandasの`area`プロパティを使うつもりでしたが、これは平面直角座標系でない限り、正確な面積とはなりません。一方、`pyproj`のGeodクラスを使うと、緯度経度（楕円体上）でも正確な面積計算ができることを知り、土地利用代表値の付与処理で活用しました。これにより、従来よりも精度の高い空間演算ができるようになり、地理空間データを扱う際の計算方法選択の重要性を改めて認識しました。
